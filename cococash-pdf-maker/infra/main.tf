...
resource "aws_lambda_function" "pdf_maker" {
  ...
  # Generated by null_resource.pdf_maker_build from Go source files
  ...
  environment {
    variables = {
      SQS_QUEUE_URL  = var.sqs_report_txns_queue_url
      S3_BUCKET_NAME = aws_s3_bucket.pdf_odb.bucket
    }
  }

  depends_on = [aws_cloudwatch_log_group.pdf_maker, null_resource.pdf_maker_build]
...
# Build Go Binary
# -----------------------------------------------
resource "null_resource" "pdf_maker_build" {
  triggers = {
